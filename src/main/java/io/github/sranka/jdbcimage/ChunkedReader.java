...
public class ChunkedReader extends Reader {
    ...
    public ChunkedReader(char[] chunk) {
        this(Collections.singletonList(chunk), chunk.length);
    }
    ...
    public String readAsString() {
        if (finished) {
            ...
            StringBuilder buffer = new StringBuilder((int) totalLength);
            for (; ; ) {
                if (this.currentChunk != null) buffer.append(this.currentChunk);
                if (chunks.hasNext()) {
                    this.currentChunk = chunks.next();
                } else {
                    break;
                }
            }
